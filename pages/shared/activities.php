...
if ($activitiesAvailable) {
    ...
    $stmt = $conn->prepare("SELECT activity_date, activity_time, title, venue FROM research_activities WHERE status = 'scheduled' AND activity_date >= CURDATE() ORDER BY activity_date ASC, activity_time ASC, activity_id ASC LIMIT 5");
...
<style>
.module-hub{max-width:1320px;margin:0 auto;color:#172033}.module-intro{display:flex;justify-content:space-between;gap:24px;align-items:end;margin:0 0 24px;padding:28px 30px;border-left:5px solid #0f766e;border-radius:4px 18px 18px 4px;background:#eef8f6}.module-kicker{margin:0 0 7px;color:#0f766e;font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.module-intro h1{margin:0;font-size:clamp(25px,3vw,38px);line-height:1.08;letter-spacing:-.035em}.module-intro p{max-width:650px;margin:10px 0 0;color:#526176;line-height:1.6}.stat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:24px}.stat-card{padding:20px 22px;border:1px solid #dfe7e5;border-radius:14px;background:#fff}.stat-card strong{display:block;font-size:30px;line-height:1;font-variant-numeric:tabular-nums}.stat-card span{display:block;margin-top:8px;color:#64748b;font-size:12px;font-weight:700}.hub-panel{margin-bottom:24px;border:1px solid #e2e8f0;border-radius:16px;background:#fff;box-shadow:0 12px 30px rgba(30,50,70,.05)}.panel-head{display:flex;justify-content:space-between;gap:18px;align-items:center;padding:18px 20px;border-bottom:1px solid #e8edf3}.panel-head h2{margin:0;font-size:17px;letter-spacing:-.01em}.panel-head p{margin:4px 0 0;color:#64748b;font-size:12px}.upcoming-list{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:0}.upcoming-item{padding:18px 20px;min-width:0}.upcoming-item+.upcoming-item{border-left:1px solid #edf1f5}.upcoming-date{color:#0f766e;font-size:11px;font-weight:800;text-transform:uppercase}.upcoming-item strong{display:block;margin-top:7px;font-size:13px;line-height:1.4}.upcoming-item span{display:block;margin-top:5px;color:#64748b;font-size:11px}.filter-form{display:grid;grid-template-columns:minmax(220px,1fr) 170px 170px auto;gap:12px;align-items:end;padding:20px}.field label{display:block;margin-bottom:6px;color:#526176;font-size:11px;font-weight:800}.field input,.field select{width:100%;min-height:43px;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:9px;padding:9px 11px;background:#fff;color:#172033;font:inherit;font-size:13px}.field input:focus,.field select:focus{border-color:#0f766e;outline:3px solid rgba(15,118,110,.12)}.filter-actions{display:flex;gap:8px}.hub-button{display:inline-flex;align-items:center;justify-content:center;min-height:43px;box-sizing:border-box;border:1px solid #0f766e;border-radius:9px;padding:9px 15px;background:#0f766e;color:#fff;font:inherit;font-size:12px;font-weight:800;text-decoration:none;cursor:pointer;transition:transform .18s ease,background .18s ease}.hub-button:hover{background:#115e59;color:#fff;transform:translateY(-1px)}.hub-button.secondary{border-color:#cbd5e1;background:#fff;color:#475569}.table-wrap{overflow-x:auto}.hub-table{width:100%;min-width:850px;border-collapse:collapse}.hub-table th{padding:12px 16px;background:#f8fafc;color:#64748b;text-align:left;font-size:10px;letter-spacing:.06em;text-transform:uppercase}.hub-table td{padding:15px 16px;border-top:1px solid #edf1f5;font-size:13px;vertical-align:middle}.title-cell{font-weight:750;color:#172033}.muted{color:#64748b}.badge{display:inline-flex;border-radius:999px;padding:5px 9px;font-size:10px;font-weight:800;white-space:nowrap}.tone-info{background:#e0f2fe;color:#075985}.tone-success{background:#dcfce7;color:#166534}.tone-danger{background:#fee2e2;color:#991b1b}.tone-neutral{background:#e2e8f0;color:#475569}.empty-state{padding:48px 24px;text-align:center}.empty-mark{display:grid;place-items:center;width:48px;height:48px;margin:0 auto 14px;border-radius:13px;background:#e3f3f0;color:#0f766e;font-size:21px}.empty-state h2{margin:0;font-size:17px}.empty-state p{max-width:480px;margin:7px auto 0;color:#64748b;font-size:13px;line-height:1.55}@media(max-width:980px){.upcoming-list{grid-template-columns:repeat(2,1fr)}.upcoming-item+.upcoming-item{border-left:0;border-top:1px solid #edf1f5}.filter-form{grid-template-columns:1fr 1fr}}@media(max-width:640px){.module-intro{padding:24px 21px}.stat-grid{grid-template-columns:1fr}.upcoming-list,.filter-form{grid-template-columns:1fr}.filter-actions,.hub-button{width:100%}}
</style>
...
<div class="module-hub">
    ...
    <?php else: ?>
      <div class="upcoming-list">
        <?php foreach ($upcoming as $item): ?>
          <article class="upcoming-item"><div class="upcoming-date"><?= activitiesEscape(date('M j, Y', strtotime((string) $item['activity_date']))) ?></div><strong><?= activitiesEscape($item['title']) ?></strong><span><?= activitiesEscape($item['venue'] ?: 'Venue to be announced') ?></span></article>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
  </section>
  ...
  <section class="hub-panel" aria-labelledby="activity-directory-heading">
    ...
    <?php else: ?>
      <div class="table-wrap"><table class="hub-table"><thead><tr><th>Date</th><th>Title</th><th>Type</th><th>Venue</th><th>Organizer</th><th>Status</th></tr></thead><tbody>
      <?php foreach ($activities as $activity): $badge = activitiesStatusBadge((string) $activity['status']); ?><tr><td><?= activitiesEscape(date('M j, Y', strtotime((string) $activity['activity_date']))) ?></td><td class="title-cell"><?= activitiesEscape($activity['title']) ?></td><td><?= activitiesEscape($typeOptions[$activity['activity_type']] ?? ucwords(str_replace('_', ' ', (string) $activity['activity_type']))) ?></td><td class="muted"><?= activitiesEscape($activity['venue'] ?: '—') ?></td><td class="muted"><?= activitiesEscape($activity['organizer'] ?: '—') ?></td><td><span class="badge tone-<?= activitiesEscape($badge['tone']) ?>"><?= activitiesEscape($badge['label']) ?></span></td></tr><?php endforeach; ?>
      </tbody></table></div>
    <?php endif; ?>
  </section>
</div>
