...
if ($publicationsAvailable) {
    ...
    $sql = 'SELECT publication_id, research_title, researcher_name, researcher_category, publication_type, journal_publisher, publication_date, status FROM publications WHERE 1 = 1';
    $params = [];
    $types = '';
    if ($ownScope) {
    ...
    if ($ownScope) {
        $stmt = $conn->prepare("SELECT research_title, researcher_name, journal_publisher, publication_date FROM publications WHERE status = 'published' ORDER BY publication_date DESC, publication_id DESC LIMIT 5");
...
<style>
.module-hub{max-width:1320px;margin:0 auto;color:#172033}.module-intro{display:flex;justify-content:space-between;gap:24px;align-items:end;margin:0 0 24px;padding:28px 30px;border-left:5px solid #315b9f;border-radius:4px 18px 18px 4px;background:#eef3fb}.module-kicker{margin:0 0 7px;color:#315b9f;font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.module-intro h1{margin:0;font-size:clamp(25px,3vw,38px);line-height:1.08;letter-spacing:-.035em}.module-intro p{max-width:650px;margin:10px 0 0;color:#526176;line-height:1.6}.repository-link{display:inline-flex;align-items:center;min-height:42px;padding:0 15px;border:1px solid #315b9f;border-radius:9px;color:#315b9f;font-size:12px;font-weight:800;text-decoration:none;white-space:nowrap;transition:transform .18s ease,background .18s ease}.repository-link:hover{background:#fff;color:#24477e;transform:translateY(-1px)}.stat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:24px}.stat-card{padding:20px 22px;border:1px solid #dfe5ee;border-radius:14px;background:#fff}.stat-card strong{display:block;font-size:30px;line-height:1;font-variant-numeric:tabular-nums}.stat-card span{display:block;margin-top:8px;color:#64748b;font-size:12px;font-weight:700}.hub-panel{margin-bottom:24px;border:1px solid #e2e8f0;border-radius:16px;background:#fff;box-shadow:0 12px 30px rgba(30,50,70,.05)}.panel-head{display:flex;justify-content:space-between;gap:18px;align-items:center;padding:18px 20px;border-bottom:1px solid #e8edf3}.panel-head h2{margin:0;font-size:17px}.panel-head p{margin:4px 0 0;color:#64748b;font-size:12px}.filter-form{display:grid;grid-template-columns:minmax(220px,1fr) 170px 180px auto;gap:12px;align-items:end;padding:20px}.field label{display:block;margin-bottom:6px;color:#526176;font-size:11px;font-weight:800}.field input,.field select{width:100%;min-height:43px;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:9px;padding:9px 11px;background:#fff;color:#172033;font:inherit;font-size:13px}.field input:focus,.field select:focus{border-color:#315b9f;outline:3px solid rgba(49,91,159,.12)}.filter-actions{display:flex;gap:8px}.hub-button{display:inline-flex;align-items:center;justify-content:center;min-height:43px;box-sizing:border-box;border:1px solid #315b9f;border-radius:9px;padding:9px 15px;background:#315b9f;color:#fff;font:inherit;font-size:12px;font-weight:800;text-decoration:none;cursor:pointer;transition:transform .18s ease,background .18s ease}.hub-button:hover{background:#24477e;color:#fff;transform:translateY(-1px)}.hub-button.secondary{border-color:#cbd5e1;background:#fff;color:#475569}.table-wrap{overflow-x:auto}.hub-table{width:100%;min-width:980px;border-collapse:collapse}.hub-table th{padding:12px 16px;background:#f8fafc;color:#64748b;text-align:left;font-size:10px;letter-spacing:.06em;text-transform:uppercase}.hub-table td{padding:15px 16px;border-top:1px solid #edf1f5;font-size:13px;vertical-align:middle}.title-cell{max-width:320px;font-weight:750;line-height:1.4}.muted{color:#64748b}.badge{display:inline-flex;border-radius:999px;padding:5px 9px;font-size:10px;font-weight:800;white-space:nowrap}.tone-warning{background:#fef3c7;color:#92400e}.tone-review{background:#ede9fe;color:#5b21b6}.tone-info{background:#cffafe;color:#155e75}.tone-success{background:#dcfce7;color:#166534}.tone-neutral{background:#e2e8f0;color:#475569}.recent-list{display:grid;grid-template-columns:repeat(5,minmax(0,1fr))}.recent-item{padding:18px 20px;min-width:0}.recent-item+.recent-item{border-left:1px solid #edf1f5}.recent-item strong{display:block;font-size:13px;line-height:1.45}.recent-item span{display:block;margin-top:6px;color:#64748b;font-size:11px;line-height:1.45}.empty-state{padding:48px 24px;text-align:center}.empty-mark{display:grid;place-items:center;width:48px;height:48px;margin:0 auto 14px;border-radius:13px;background:#e8eef9;color:#315b9f;font-size:21px}.empty-state h2{margin:0;font-size:17px}.empty-state p{max-width:480px;margin:7px auto 0;color:#64748b;font-size:13px;line-height:1.55}@media(max-width:980px){.filter-form{grid-template-columns:1fr 1fr}.recent-list{grid-template-columns:repeat(2,1fr)}.recent-item+.recent-item{border-left:0;border-top:1px solid #edf1f5}}@media(max-width:640px){.module-intro{display:block;padding:24px 21px}.repository-link{margin-top:18px}.stat-grid,.filter-form,.recent-list{grid-template-columns:1fr}.filter-actions,.hub-button{width:100%}}
</style>

<div class="module-hub">
  <header class="module-intro"><div><p class="module-kicker">Research dissemination</p><h1>Publication record</h1><p><?= publicationsEscape($ownScope ? 'Track your submitted work while keeping sight of recently published institutional research.' : 'Monitor faculty and student publication outputs from submission through publication.') ?></p></div><a class="repository-link" href="<?= publicationsEscape(SITE_URL . 'pages/shared/research-archive.php') ?>">Open Repository</a></header>
